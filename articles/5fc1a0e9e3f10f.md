...
module OrderValidation
  ...
  included do
    # includeしただけで、暗黙的にデータが書き換わるようになる
...
Form Objectで保存前の処理を明示的に書くのが良いと思います。
...
- モジュールの冒頭に副作用を明記し、READMEやコメントで注意を促す
- どうしてもコールバックを使う場合は、命名を副作用が分かりやすい動詞にする
:::
...
module OrderCalculatable
  ...
  def calculate_tax
    # `items` や `user` が include先に存在しないとエラーになる
...
class User
  ...
  def initialize(name)
    @name = name
    super # ここでsuperを忘れると不具合になる可能性に繋がる
  end
end
```
...
class User
  ...
  def initialize(name)
    @name = name
    super # ここで Loggable#initialize は呼ばれるが...
  end
end
...
class User < ApplicationRecord
  include A # Cのメソッドが使えるが、どこから来たか追いづらい
end
```
...
# テストのためだけにDB接続が必要なダミーモデルを作る必要がある
...
* 「共通だから」ではなく「何をする役割か」で名前を付ける（例: `DateFormattable`, `CsvExportable`）。
* もし適切な名前が思いつかないなら、それは単一のモジュールにするべきではない複数の責務が混ざっている証拠です。分解しましょう。
