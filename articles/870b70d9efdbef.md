...
私は普段、10年以上運用されている大規模なRailsアプリケーションに携わっており、そこではレイヤードアーキテクチャに近しいものが採用されています。そのためよりレイヤードアーキテクチャの理解を深めるために読みました。英語のリーディング練習も兼ねています。翻訳や内容の整理には、かなりAIも頼っています。
...
改めて参考になったのは、**「ドメインのルール（不変条件）」と「アプリケーションの手順（ユースケース）」を分けて考える視点**です。
...
ここからは、私のこれまでの経験から加えられそうなアレンジを紹介します。レイヤーの境界を決めるだけでなく、依存方向や隠れた副作用を継続的に確認するためのガードレールです。
...
class PublishedPostsQuery
  def self.call
    Post.where(status: :published).strict_loading.includes(:author)
  end
end
```
