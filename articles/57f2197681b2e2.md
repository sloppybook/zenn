...
テーブル設計において熟考されていないフラグ追加は、データ整合性の低下、検索パフォーマンスの劣化、および情報の損失を招く恐れがあります。
...
これらの課題を、以下の3つのステップで解決していきます。


## 🤔構造の分離：別テーブルにできないか

まずは**状態によって守るべきルール（制約）が異なるか**という点を検討しました。
...
erDiagram
  ...
  articles {
    intger id PK "リソースの識別子"
    intger user_id FK "作成者"
    timestamp created_at "発生日時"
  }

  article_events {
    bigint id PK
    intger article_id FK "articles.id"
    int event_type "Enum (created | updated | published | archived)"
    jsonb payload "その時のタイトルや本文の差分"
    intger actor_id FK "実行者"
...
複数のフラグで管理しようとすると、論理的にあり得ない状態が生まれてしまいます。
...
erDiagram
  articles {
    intger id PK
    intger user_id FK "NOT NULL"
    intger status "Enum (draft / published / archived)"
    varchar title "ALLOW NULL"
    text content "ALLOW NULL"
    timestamp created_at
    timestamp updated_at
...
設計時には、以下の3段階のフィルタを優先順位に従って通すことを推奨します。

* 構造の分離（別テーブル）： 制約（ルール）が違うなら、物理的に分ける。
* 時間軸の導入（日付・履歴）： 出来事の証跡が必要なら、事実を記録する。
* 状態の集約（Enum）： 排他的な状態なら、一つにまとめる。

これらを検討し、することで、数年後の自分やチームメンバーが「メンテナンスしやすい」と感じる、真に運用耐性の高いテーブル構造に繋がればと思います。