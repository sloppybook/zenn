...
最近業務にてテーブル設計をレビューする際にフラグ一つで色々思いふけることがあったので言語化しました。
数年前までは何も考えずにフラグを追加していたので少し成長を感じました。😊
...
## おまけ: 履歴テーブルへの切り出し
「時間軸の導入」「状態の集約」の発展系として、「いつ」「どの状態」になったか、またその状態に戻したいケースが考えられます。
もしそのようなプロセス自体にビジネス価値がある場合は、UPDATEではなくINSERTによる**イミュータブルな事実の記録**を検討します。
